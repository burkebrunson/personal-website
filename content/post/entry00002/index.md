...
One customization I made is thatI changed the title of the `Posts` page to `Blog` (and made the necessary updates to the name in the navigation menu, too). I also changed the style of `View` for the posts page to `Card`.

You're basically done setting up your personal website and/or blog, at this point. However, there are some quick key customizations I think you should consider detailed next, so read on.

## (Optional) No blog widget on the homepage?
